...
export const PostItem = ({ post }: Props) => {
  return (
    <div className="relative group">
      <div className="absolute -inset-1 rounded-[20px] bg-gradient-to-r from-blue-600 to-blue-300 blur-sm opacity-0 group-hover:opacity-50 transition duration-300 pointer-events-none"></div>
      <Link to={`/post/${post.title}/${post.id}`} className="block relative z-10">
        <div className="w-80 h-76 bg-[rgb(24,27,32)] border border-[rgb(84,90,106)] rounded-[20px] text-white flex flex-col p-5 overflow-hidden transition-colors duration-300 group-hover:bg-gray-800">
          ...
          {/* Image Banner (if image_url is set) */}
          <div className="mt-2 flex-1">
            {/* set the image. If no image make the 'alt' just the first 50 characters of post.content */}
            <img
              src={post.image_url}
              className="w-full rounded-[20px] object-cover max-h-[150px] mx-auto"
              alt={ post.content.length > 50 ? post.content.substring(0, 50) + "..." : post.content}
            />
          </div>
